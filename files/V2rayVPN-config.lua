...
m = Map("V2rayVPN", translate("配置文件"), translate("V2Ray服务器配置, 目前只支持 vmess + websocket,【socks代理端口】开启socks代理，【http代理端口】开启http代理"))
...
protocol = s:option(Value, "protocol", translate("协议默认ws"))
...
if apply then
    io.popen("/etc/V2rayVPN/genConfig start ; /etc/init.d/V2rayVPN restart")
end
